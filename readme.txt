Mochi Group

Aaron Ashmore - Leader
Myranda Estes - Member

API (Ideas):
 Food Recipes - https://developer.edamam.com/edamam-docs-recipe-api
 Dogs - https://dog.ceo/dog-api/?ref=apilist.fun
 Dungeons and Dragons - https://5e-bits.github.io/docs/
 Magic The Gathering - https://scryfall.com/docs/api

Description:
 Food - Website that gives recipes of various foods.
 Dogs - Website that shows info over dogs.
 Dungeons and Dragons - Website that allows you to create a character.
 Magic The Gathering - Website that rates playing cards.

Site Structure:
Index - Links to other pages. Banner for our topic. Hero Image
Content - The main page to show (Recipes/Dogs/DND/Magic) information.
     ie. Food Recipe instructions
     ie. Dogs facts 
     ie. DND Character Creator
     ie. Magic Card Ratings
Contact Us - Page to input you Name, Phone, Email, ect.
About - Gives a description of what the website is about and some info about us.
Gallery - Image gallery of content.
